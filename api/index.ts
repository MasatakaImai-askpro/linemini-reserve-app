...
function sql(strings: TemplateStringsArray, ...values: any[]): Promise<any[]> {
  let query = "";
  strings.forEach((str, i) => { query += str; if (i < values.length) query += `$${i + 1}`; });
  return _pool.query(query, values).then(r => r.rows);
}
...
function toReservation(r: any) { return { id: r.id.toString(), customerName: r.customer_name, customerPhone: r.customer_phone || undefined, customerEmail: r.customer_email || undefined, date: r.date, time: r.time, staffId: r.staff_id || "__shop__", courseId: r.course_id, partySize: r.party_size ?? 1, status: r.status || "confirmed", paid: r.paid || false, reservationToken: r.cancel_token || undefined, notes: r.notes || undefined }; }
...
export function ensureSetup(): Promise<void> {
  if (!_setup) {
    _setup = (async () => {
      ...
      // ─── スロット ───
      // ─── スロット（お客様向け：日付指定で空き時間を返す） ───
      const slotTimeToMin = (t: string) => { const [h,m] = t.split(":").map(Number); return h*60+m; };
      app.get("/api/shops/:shopId/slots", async (req, res) => {
        ...
        try {
          ...
          const stRows = await safeQuery(() => sql`SELECT open_time, close_time, closed_dow, closed_newyear, table_count FROM booking_settings WHERE shop_id=${shopId}`);
          const st = stRows[0];
          const openMin = slotTimeToMin(st?.open_time || "10:00");
          const closeMin = slotTimeToMin(st?.close_time || "19:00");
          const closedDow = (st?.closed_dow || "").split(",").filter(Boolean).map(Number);
          const closedNY = st?.closed_newyear === true;
          const allSlots: string[] = [];
          for (let m = openMin; m < closeMin; m += 30) allSlots.push(`${String(Math.floor(m/60)).padStart(2,"0")}:${String(m%60).padStart(2,"0")}`);
          if (!date) return res.json(allSlots.map(t => ({ time: t, available: true })));
          const [dy,dm,dd] = date.split("-").map(Number);
          const dow = new Date(Date.UTC(dy, dm-1, dd)).getUTCDay();
          if (closedDow.includes(dow)) return res.json(allSlots.map(t => ({ time: t, available: false })));
          if (closedNY && ((dm===12&&dd>=29)||(dm===1&&dd<=3))) return res.json(allSlots.map(t => ({ time: t, available: false })));
          ...
          const dateOverrides = await safeQuery(() => sql`SELECT time FROM booking_slot_dates WHERE shop_id=${shopId} AND date=${date} AND staff_id='__shop__' AND available=false`);
          const forcedClosed = new Set(dateOverrides.map((r: any) => r.time as string));
          ...
          res.json(allSlots.map(slot => { const [sh,sm]=slot.split(":").map(Number); const sStart=sh*60+sm; if (forcedClosed.has(slot)) return {time:slot,available:false}; if (date===todayJST && nowMinutes >= sStart-15) return {time:slot,available:false}; let max=0; for(let i=0;i<slotsNeeded;i++){const cm=sStart+i*30;const cs=`${String(Math.floor(cm/60)).padStart(2,"0")}:${String(cm%60).padStart(2,"0")}`;max=Math.max(max,slotCount.get(cs)||0);} return {time:slot,available:max<tableCount}; }));
        } catch (e: any) { console.error("slots error:", e); res.status(500).json({ message: "Failed to fetch slots" }); }
      });

      // ─── 日付単位スロット上書き（管理者向け） ───
      app.get("/api/shops/:shopId/slot-dates", async (req, res) => {
        const shopId = parseInt(req.params.shopId); if (isNaN(shopId)) return res.status(400).json({ message: "Invalid shop ID" });
        try {
          const { staffId, from, to } = req.query as Record<string,string>;
          if (!staffId || !from || !to) return res.status(400).json({ message: "staffId, from, to required" });
          const rows = await safeQuery(() => sql`SELECT date::text, time, available FROM booking_slot_dates WHERE shop_id=${shopId} AND staff_id=${staffId} AND date >= ${from}::date AND date <= ${to}::date`);
          res.json(rows.map((r: any) => ({ date: r.date, time: r.time, available: r.available })));
        } catch (e: any) { res.status(500).json({ message: "Failed to fetch slot dates" }); }
      });
      app.put("/api/shops/:shopId/slot-dates", async (req, res) => {
        const shopId = parseInt(req.params.shopId); if (isNaN(shopId)) return res.status(400).json({ message: "Invalid shop ID" });
        try {
          const { staffId, date, time, available } = req.body;
          await sql`INSERT INTO booking_slot_dates (shop_id, staff_id, date, time, available) VALUES (${shopId}, ${staffId}, ${date}::date, ${time}, ${available}) ON CONFLICT (shop_id, staff_id, date, time) DO UPDATE SET available=${available}`;
          res.json({ ok: true });
        } catch (e: any) { res.status(500).json({ message: "Failed to update slot date" }); }
      });
      app.post("/api/shops/:shopId/slot-dates/bulk", async (req, res) => {
        const shopId = parseInt(req.params.shopId); if (isNaN(shopId)) return res.status(400).json({ message: "Invalid shop ID" });
        try {
          const { staffId, date, times, available } = req.body;
          if (!Array.isArray(times) || times.length === 0) return res.status(400).json({ message: "times required" });
          for (const time of times) {
            await sql`INSERT INTO booking_slot_dates (shop_id, staff_id, date, time, available) VALUES (${shopId}, ${staffId}, ${date}::date, ${time}, ${available}) ON CONFLICT (shop_id, staff_id, date, time) DO UPDATE SET available=${available}`;
          }
          res.json({ ok: true });
        } catch (e: any) { res.status(500).json({ message: "Failed to bulk update slot dates" }); }
      });
      ...
      app.get("/api/shops/:shopId/settings", async (req, res) => {
        ...
        try {
          ...
          if (parseInt(cnt[0]?.cnt||"0")>0) {
            const rows = await sql`SELECT * FROM booking_settings WHERE shop_id=${shopId}`; const s=rows[0];
            return res.json({store_name:s.store_name||"",store_description:s.store_description||"",store_address:s.store_address||"",store_phone:s.store_phone||"",store_email:s.store_email||"",store_hours:s.store_hours||"",store_closed_days:s.store_closed_days||"",banner_url:s.banner_url||"",staff_selection_enabled:s.staff_selection_enabled||"false",table_count:s.table_count!=null?String(s.table_count):"0",max_party_size:s.max_party_size!=null?String(s.max_party_size):"0",open_time:s.open_time||"10:00",close_time:s.close_time||"19:00",closed_dow:s.closed_dow||"",closed_newyear:s.closed_newyear?"true":"false"});
          }
      ...
      app.put("/api/shops/:shopId/settings", async (req, res) => {
        ...
        try {
          ...
          await sql`INSERT INTO booking_settings (shop_id,store_name,store_description,store_address,store_phone,store_email,store_hours,store_closed_days,banner_url,staff_selection_enabled,table_count,max_party_size,open_time,close_time,closed_dow,closed_newyear,updated_at) VALUES (${shopId},${s.store_name||""},${s.store_description||""},${s.store_address||""},${s.store_phone||""},${s.store_email||""},${s.store_hours||""},${s.store_closed_days||""},${s.banner_url||""},${s.staff_selection_enabled||"false"},${tc},${mp},${s.open_time||"10:00"},${s.close_time||"19:00"},${s.closed_dow||""},${ny},NOW()) ON CONFLICT (shop_id) DO UPDATE SET store_name=${s.store_name||""},store_description=${s.store_description||""},store_address=${s.store_address||""},store_phone=${s.store_phone||""},store_email=${s.store_email||""},store_hours=${s.store_hours||""},store_closed_days=${s.store_closed_days||""},banner_url=${s.banner_url||""},staff_selection_enabled=${s.staff_selection_enabled||"false"},table_count=${tc},max_party_size=${mp},open_time=${s.open_time||"10:00"},close_time=${s.close_time||"19:00"},closed_dow=${s.closed_dow||""},closed_newyear=${ny},updated_at=NOW()`;
          res.json(s);
      ...
      app.post("/api/shops/:shopId/reservations", async (req, res) => {
        const shopId = parseInt(req.params.shopId); if (isNaN(shopId)) return res.status(400).json({message:"Invalid shop ID"});
        try {
          const {customerName,customerPhone,customerEmail,date,time,staffId,courseId,partySize,notes}=req.body;
          if (!customerName||!date||!time||!courseId) return res.status(400).json({message:"Missing required fields"});
          const token=crypto.randomUUID().replace(/-/g,"");
          await sql`INSERT INTO booking_reservations (shop_id,customer_name,customer_phone,customer_email,date,time,staff_id,course_id,party_size,status,paid,cancel_token,notes) VALUES (${shopId},${customerName},${customerPhone||null},${customerEmail||null},${date},${time},${staffId||"__shop__"},${courseId.toString()},${partySize||1},'confirmed',false,${token},${notes||null})`;
      ...
      app.put("/api/shops/:shopId/reservations", async (req, res) => {
        const shopId = parseInt(req.params.shopId); if (isNaN(shopId)) return res.status(400).json({message:"Invalid shop ID"});
        try {
          const {id,status,paid,customerName,customerPhone,customerEmail,date,time,partySize,notes}=req.body;
          ...
          if (partySize!==undefined) await sql`UPDATE booking_reservations SET party_size=${partySize} WHERE id=${resId} AND shop_id=${shopId}`;
          if (notes!==undefined) await sql`UPDATE booking_reservations SET notes=${notes||null} WHERE id=${resId} AND shop_id=${shopId}`;
      ...
      app.get("/robots.txt", (_req, res) => {
        const base = process.env.APP_URL || "https://linemini-reserve-app.vercel.app";
...
`);
      ...
      app.get("/sitemap.xml", async (_req, res) => {
        try {
          const shops = await sql`SELECT id, updated_at FROM shops WHERE is_active = true ORDER BY id`;
          const base = process.env.APP_URL || "https://linemini-reserve-app.vercel.app";
          const today = new Date().toISOString().split("T")[0];
          const areas = ["atsugi","odawara","ebina","sagamihara","yamato","hadano","hiratsuka","chigasaki","fujisawa","kamakura","yokohama","isehara","ninomiya","minamiashigara"];
          const categories = ["gourmet","beauty","shopping","leisure","service","medical"];
          const staticUrls = [
            { loc: base, priority: "1.0", changefreq: "daily" },
            { loc: `${base}/app`, priority: "0.95", changefreq: "daily" },
            { loc: `${base}/app/list`, priority: "0.9", changefreq: "daily" },
            ...areas.map(a => ({ loc: `${base}/app/list?area=${a}`, priority: "0.8", changefreq: "weekly" })),
            ...categories.map(c => ({ loc: `${base}/app/list?category=${c}`, priority: "0.8", changefreq: "weekly" })),
            { loc: `${base}/app/list?area=atsugi&category=gourmet`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=atsugi&category=beauty`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=odawara&category=gourmet`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=odawara&category=beauty`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=ebina&category=gourmet`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=ebina&category=beauty`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=sagamihara&category=gourmet`, priority: "0.75", changefreq: "weekly" },
            { loc: `${base}/app/list?area=sagamihara&category=beauty`, priority: "0.75", changefreq: "weekly" },
          ];
          const shopUrls = shops.map((s: any) => ({
            loc: `${base}/app/shop/${s.id}`,
...
${allUrls.map(u => `  <url>
    <loc>${(u as any).loc}</loc>
    <lastmod>${(u as any).lastmod || today}</lastmod>
    <changefreq>${(u as any).changefreq}</changefreq>
    <priority>${(u as any).priority}</priority>
  </url>`).join("\n")}
</urlset>`);
        } catch (e: any) { res.status(500).send("Sitemap generation failed"); }
      });

      // ─── LLMO: llms.txt ───
      app.get("/llms.txt", async (_req, res) => {
        try {
          const shops = await sql`SELECT id, name, area, category, subcategory, description, address FROM shops WHERE is_active = true ORDER BY display_order DESC, id LIMIT 60`;
          const base = process.env.APP_URL || "https://linemini-reserve-app.vercel.app";
          res.set("Content-Type", "text/plain; charset=utf-8");
          res.set("Cache-Control", "public, max-age=3600");
          res.send(`# かながわスマイルマップ
> 神奈川県（厚木・小田原・海老名・相模原）の地域密着型店舗ポータル。飲食店・美容室・エステサロンのオンライン予約とLINEクーポンを提供するLINEミニアプリ。

## サービス概要
- サービス名: かながわスマイルマップ
- URL: ${base}
- 対象エリア: 神奈川県（厚木市・小田原市・海老名市・相模原市）
- 対象駅: 本厚木駅・小田原駅・海老名駅・相模大野駅・橋本駅・相模原駅・町田駅
- 対象業種: 飲食店・ラーメン・居酒屋・カフェ・和食・美容室・ヘアサロン・エステサロン・ネイル
- 主要機能: オンライン予約・LINEクーポン・Google Maps連携・LINEミニアプリ対応

## 対象キーワード
厚木 飲食店 予約, 厚木 美容室 クーポン, 本厚木 ラーメン, 本厚木 居酒屋, 本厚木 エステ,
小田原 飲食店 予約, 小田原 美容室, 小田原 エステサロン, 小田原 ラーメン,
海老名 飲食店, 海老名 ラーメン, 海老名 美容室 予約, 海老名 居酒屋,
相模原 飲食店, 相模大野 居酒屋, 橋本 ラーメン, 相模原 美容室,
神奈川 エステサロン 予約, 神奈川 美容室 クーポン, 神奈川 居酒屋, 神奈川 ラーメン

## 掲載店舗一覧
${shops.map((s: any) => `### ${s.name}
- エリア: ${s.area}
- 業種: ${s.category}${s.subcategory ? `（${s.subcategory}）` : ""}
- 住所: ${s.address}
- 説明: ${s.description}
- 予約・詳細: ${base}/app/shop/${s.id}`).join("\n\n")}

## よくある質問
### 厚木・本厚木エリアのラーメン店・居酒屋を予約するには？
かながわスマイルマップのエリア選択で「厚木」を選び、業種「グルメ」でラーメン・居酒屋を検索できます。オンライン予約とクーポン取得が可能です。

### 小田原の美容室・エステサロンをクーポン付きで予約するには？
小田原エリアの美容室・エステサロンを一覧で比較し、LINE限定クーポンやWEB共通クーポンを取得してからオンライン予約できます。

### 海老名・海老名駅周辺の飲食店情報を見るには？
海老名エリアフィルターを選択するとラーメン・居酒屋・カフェ等の飲食店一覧が表示されます。

### 相模大野・橋本エリアの飲食店・美容室は？
相模原市（相模大野・橋本・相模原駅周辺）の飲食店・美容室をエリア検索で絞り込めます。

### LINEミニアプリとして使えますか？
はい。LINEアプリ内からインストール不要でそのまま利用できます。LINE公式アカウントを友だち追加するとLINE限定クーポンも受け取れます。

### 予約のキャンセル方法は？
予約完了メールに記載のキャンセルリンクから、ログイン不要でいつでもキャンセルできます。
...
- 対応プラットフォーム: LINEミニアプリ・WEB（スマートフォン・PC）
`);
        } catch (e: any) { res.status(500).send("llms.txt generation failed"); }
      });
