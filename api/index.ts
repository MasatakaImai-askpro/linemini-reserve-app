    ...
    _setup = (async () => {
      ...
      app.get("/api/shops/:shopId/slots", async (req, res) => {
        ...
        try {
          ...
          res.json(allSlots.map(slot => { const [sh,sm]=slot.split(":").map(Number); const sStart=sh*60+sm; if (sStart+courseDuration>19*60) return {time:slot,available:false}; if (date===todayJST && nowMinutes >= sStart-15) return {time:slot,available:false}; let max=0; for(let i=0;i<slotsNeeded;i++){const cm=sStart+i*30;const cs=`${String(Math.floor(cm/60)).padStart(2,"0")}:${String(cm%60).padStart(2,"0")}`;max=Math.max(max,slotCount.get(cs)||0);} return {time:slot,available:max<tableCount}; }));
